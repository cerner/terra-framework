...
import DraggableTabs from './example/DraggableTabs?dev-site-example';
import TabsCloseFunc from './example/TabsCloseFunc?dev-site-example'
...
When a draggable tab has focus press the `Spacebar` key to start the drag. Once a drag is started, the following keyboard shortcuts can be used:
* `Spacebar` - Drop the tab
* `Escape` - Cancel the drag
* `Left arrow` - move the tab to the left.
* `Right arrow` - move the tab to the right.
...
<DraggableTabs title='Draggable Tabs' />
<TabsCloseFunc title='Closable Tabs' />
...
Tabs uses `uuid` which changes the component's description id dynamically. To mock it with the Jest testing library, `jest.mock` can be used.
