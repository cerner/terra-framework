...
import AddTab from './example/AddTab?dev-site-example'
...
Tabs uses `uuid` which changes the component's description id dynamically. To mock the return value with the Jest testing library, `jest.spyOn` can be used.
