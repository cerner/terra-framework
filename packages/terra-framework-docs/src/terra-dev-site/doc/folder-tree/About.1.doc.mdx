...
The tree part of the component contains only one tab stop with the arrow keys used for primary navigation.
...
* Folder Tree does not manage consumer data state. Expanded/collapsed and selection state of rows
must be managed by the consumer.
