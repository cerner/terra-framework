...
The Flowsheet Data Grid is an [accessible grid](https://www.w3.org/WAI/ARIA/apg/patterns/grid/) component that allows the user to display data in a table-like structure of rows and columns.
...
|Escape| Removes all cell selections.<br/> When grid navigation is off, escape restores the grid navigation.|
|Shift + Up|Extends cell selection one row up. The selection does not extend beyond the first non-header row.|
...
|Shift + Down|Extends cell selection one column to the right. The selection does not extend beyond the last column.|
...
## Properties
<Notice variant="important" ariaLevel="2">
When making changes to the columns, section, rows, or cells, the FlowsheetDataGrid component requires you to pass new objects so that React can detect the changes.  
