...
- Filter Pills as a group requires that an aria-label be provided as accesibility minimum standard protection, but is not visible, and therefore is not reccomended to be used alone.
...
<table style={{ margin: "5px", border: "1px solid #0000", 'border-collapse': "collapse" }}>
  ...
  <tr>
    ...
    <td>Removes the pill that currently has focus. After removing, focus should then be placed on the previous pill in the list. If no pills remain, focus is temporarily on the Filter Pills group until user navigates away, but cannot be focused on again while no interactive pills remain in the Filter Pills group.</td>
  </tr>
  <tr>
