...
- The `custom` variant allows for less common standardized non-hazard situations, requiring an icon and signal word be provided in addition to the common props. By choosing to create a custom Notification Dialog, you are responsible for providing an icon that is themed correctly for the terra-default-theme, orion-fusion-theme, and clinical-lowlight-theme. See Terra’s [Theme Strategy Guide](https://engineering.cerner.com/terra-ui/guides/terra-application/theme-strategy) for more info.
...
`! IMPORTANT !` When opening the Notification Dialog over another modal dialog, please make sure to make the initial modal dialog inert, so the user cannot tab inside it.
