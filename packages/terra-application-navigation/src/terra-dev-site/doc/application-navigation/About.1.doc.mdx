...
If neither initials or the imageSrc are  provided, the avatar will fall back to the generic user avatar. If initials are not provided and imageSrc is provided, but fails, the avatar will fall back to the first two letters of the provided name. Initials will be required with the next major version of application navigation, provided via terra-application v2.
