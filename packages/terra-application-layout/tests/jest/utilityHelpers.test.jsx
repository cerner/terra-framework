...
describe('utilityHelpers', () => {
  describe('getDefaultUtilityItems', () => {
    it('returns the default set of utility items', () => {
      ...
      expect(menuItem.title).toBe('Terra.applicationLayout.utilityDefaults.menu');
      ...
      expect(userInformationItem.title).toBe('Terra.applicationLayout.utilityDefaults.userInformation');
      ...
      expect(settingsItem.title).toBe('Terra.applicationLayout.utilityDefaults.settings');
      ...
      expect(logOutItem.title).toBe('Terra.applicationLayout.utilityDefaults.logOut');

      const helpItem = result[4];
      expect(helpItem.key).toBe(utilityHelpers.defaultKeys.HELP);
      expect(helpItem.title).toBe('Terra.applicationLayout.utilityDefaults.help');
      ...
      expect(changePhotoItem.title).toBe('Terra.applicationLayout.utilityDefaults.changePhoto');

      const appearanceItem = result[6];
      expect(appearanceItem.key).toBe(utilityHelpers.defaultKeys.APPEARANCE);
      expect(appearanceItem.title).toBe('Terra.applicationLayout.utilityDefaults.appearance');

      const securityItem = result[7];
      expect(securityItem.key).toBe(utilityHelpers.defaultKeys.SECURITY);
      expect(securityItem.title).toBe('Terra.applicationLayout.utilityDefaults.security');

      const gettingStartedItem = result[8];
      expect(gettingStartedItem.key).toBe(utilityHelpers.defaultKeys.GETTING_STARTED);
      expect(gettingStartedItem.title).toBe('Terra.applicationLayout.utilityDefaults.gettingStarted');

      const aboutItem = result[9];
      expect(aboutItem.key).toBe(utilityHelpers.defaultKeys.ABOUT);
      expect(aboutItem.title).toBe('Terra.applicationLayout.utilityDefaults.about');

      const termsOfUseItem = result[10];
      expect(termsOfUseItem.key).toBe(utilityHelpers.defaultKeys.TERMS_OF_USE);
      expect(termsOfUseItem.title).toBe('Terra.applicationLayout.utilityDefaults.termsOfUse');
    });
