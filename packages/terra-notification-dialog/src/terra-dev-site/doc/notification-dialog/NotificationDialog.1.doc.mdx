...
The Notification Dialog presentations are categorizes by risk as either `hazardous` or `non-hazardous`. They include fixed icons, colors, and ANSI signal words to provide consistency in risk mitigation required by regulatory affairs and safety certifications.

- Hazardous notifications are intended to make users aware of risk or hazard, such as risk to patient safety or financial risk, and have three levels of severity: `hazard-high`, `hazard-medium` and `hazard-low`. The Hazardous notifications dialogs are rigid to provide consistent icon and signal word pairs to prevent accidental confusion by the user of the dialog's severity.

- Non-hazardous notification are intended to inform a user of system activity and required actions, but not considered to cause risk. Currently there is one pre-defined non-hazardous dialog variant of `error`.
