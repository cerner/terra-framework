    ...
    return new Promise((resolve, reject) => {
      (0, import_conventional_recommended_bump.default)(options, (err, data) => {
        ...
        if (prereleaseId) {
          ...
          resolve(import_semver3.default.inc(pkg2.version, prereleaseType, prereleaseId));
        } else {
          ...
          resolve(import_semver3.default.inc(pkg2.version, releaseType));
        }
      });
    });
  });
...
function getTarballName(pkg2) {
  const name = pkg2.name[0] === "@" ? pkg2.name.substr(1).replace(/\//g, "-") : pkg2.name;
...
var require_has_dependency_installed = __commonJS({
  "libs/commands/bootstrap/src/lib/has-dependency-installed.ts"(exports2, module2) {
    ...
    function hasDependencyInstalled(pkg2, depName, needVersion) {
      import_npmlog38.default.silly("hasDependencyInstalled", pkg2.name, depName);
      return getInstalled(pkg2).then(
        (versions) => versions.has(depName) && import_semver5.default.satisfies(versions.get(depName), needVersion)
      );
    }
    function getInstalled(pkg2) {
      if (cache.has(pkg2)) {
        return Promise.resolve(cache.get(pkg2));
      }
  ...
  "libs/commands/bootstrap/src/index.ts"(exports2, module2) {
    ...
    var BootstrapCommand = class extends Command {
      ...
      initialize() {
        ...
        chain = chain.then((filteredPackages) => {
          ...
          if (this.options.contents) {
            for (const pkg2 of filteredPackages) {
      ...
      runLifecycleInPackages(stage) {
        ...
        const mapPackageWithScript = (pkg2) => this.runPackageLifecycle(pkg2, stage).then(() => {
          tracker.completeWork(1);
        });
      ...
      getDependenciesToInstall() {
        ...
        const filteredNodes = new Map(
          this.filteredPackages.map((pkg2) => [pkg2.name, this.targetGraph.get(pkg2.name)])
        );
        ...
        if (rootSet.size) {
          ...
          actions.push(() => {
            ...
            return pulseTillDone(promise).then(
              () => (0, import_p_map_series2.default)(root, ({ name, dependents }) => {
                ...
                if (bin) {
                  return (0, import_p_map5.default)(dependents, (pkg2) => {
                    const src = this.hoistedDirectory(name);
                    return symlinkBinary(src, pkg2);
                  });
                }
              })
            ).then(() => {
          ...
          actions.push(() => {
            const candidates = root.filter((dep) => dep.dependents.length).reduce((list2, { name, dependents }) => {
              const dirs = dependents.filter((pkg2) => pkg2.nodeModulesLocation !== rootPkg.nodeModulesLocation).map((pkg2) => import_path23.default.join(pkg2.nodeModulesLocation, name));
    ...
    var AddCommand = class extends Command {
      ...
      collectPackagesToChange() {
        ...
        if (this.packageGraph.has(targetName)) {
          result = result.filter((pkg2) => pkg2.name !== targetName);
        }
        if (this.dirs.size) {
          result = result.filter((pkg2) => this.dirs.has(pkg2.location));
        }
        result = result.filter((pkg2) => {
          const deps = this.getPackageDeps(pkg2);
          if (!(targetName in deps)) {
            return true;
          }
          return getRangeToReference(this.spec, deps, pkg2.location, this.savePrefix) !== deps[targetName];
        });
        return result;
      }
      makeChanges() {
        const { name: targetName } = this.spec;
        return (0, import_p_map5.default)(this.packagesToChange, (pkg2) => {
          const deps = this.getPackageDeps(pkg2);
          const range2 = getRangeToReference(this.spec, deps, pkg2.location, this.savePrefix);
          this.logger.verbose("add", `${targetName}@${range2} to ${this.dependencyType} in ${pkg2.name}`);
          deps[targetName] = range2;
          return pkg2.serialize();
        });
      }
      getPackageDeps(pkg2) {
        let deps = pkg2.get(this.dependencyType);
        if (!deps) {
          deps = {};
          pkg2.set(this.dependencyType, deps);
        }
        return deps;
      }
      getPackageVersion() {
        ...
        return import_pacote.default.manifest(this.spec, opts.snapshot).then((pkg2) => pkg2.version);
      }
      packageSatisfied() {
        const { name, fetchSpec } = this.spec;
        const pkg2 = this.packageGraph.get(name);
        if (!pkg2) {
          return false;
        }
        if (this.spec.type === "directory" && fetchSpec === pkg2.location) {
        ...
        return import_semver5.default.intersects(pkg2.version, fetchSpec);
      }
    };
      ...
      ensureConfig() {
        ...
        if (!hasExistingPackageJson) {
          ...
          chain = chain.then(() => {
            ...
            return (0, import_devkit2.writeJsonFile)(import_path23.default.join(this.project.rootPath, "package.json"), pkg2, {
              spaces: 2
            });
          });
        } else {
  ...
  "libs/commands/link/src/index.ts"(exports2, module2) {
    ...
    var LinkCommand = class extends Command {
      ...
      initialize() {
        ...
        if (this.options.contents) {
          for (const pkg2 of this.allPackages) {
            pkg2.contents = this.options.contents;
          }
        }
  ...
  "libs/commands/publish/src/index.ts"(exports2, module2) {
    ...
    var PublishCommand = class extends Command {
      ...
      resetChanges() {
        ...
        const dirtyManifests = [this.project.manifest].concat(this.packagesToPublish).map((pkg2) => import_path23.default.relative(cwd, pkg2.manifestLocation));
      ...
      execScript(pkg2, script) {
        const scriptLocation = import_path23.default.join(pkg2.location, "scripts", script);
        try {
        ...
        const mapper = (0, import_p_pipe.default)(
          ...[
            ...
            (pkg2) => pulseTillDone(packDirectory(pkg2, pkg2.location, opts)).then((packed) => {
              tracker.verbose("packed", import_path23.default.relative(this.project.rootPath, pkg2.contents));
              tracker.completeWork(1);
              pkg2.packed = packed;
              return pkg2.refresh();
            })
